...
export function createXYZLayer(maps, url, name, maxZoom, tileSize, options) {
    if(typeof(tileSize)==='number') tileSize=[tileSize, tileSize];
        ...
        // OK
    } else if(tileSize===null || tileSize===undefined) tileSize=[256, 256];     // Default if not provided
...
export default function useLayerManager(filterState) {
  ...
  mapState.current = {
    ...
    useEffect(() => {
        ...
        map.setMapTypeId(filterState.map_background ? maps.MapTypeId.SATELLITE : maps.MapTypeId.ROADMAP);
